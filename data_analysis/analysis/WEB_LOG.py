...
status_train = pd.read_csv("/Users/chandler/Desktop/train/day=20130530/train_copy.csv")
#
# # 输出dataframe的前5行
print(status_train.head(10))
#
print("\n-----------------------------------以上输出dataframe的前10行-------------------------------------------\n")
#
# # 输出dataframe的描述信息
print(status_train.describe())
...
#
# 首先把所有缺失值替换为"S"
area_list = ['hainan', 'henan', 'qinghai', 'beijing', 'zhejiang', 'hubei', 'gansu', 'China',
             'xinjiangweihuer', 'shandong', 'hunan', 'chongqin', 'heilongjiang', 'jiangsu',
             'ningxiahuizu', 'guangdong', 'neimenggu', 'guizhou', 'shanxi_J', 'tianjing',
             'jilin', 'anhui', 'Global', 'guangxizhuangzu', 'sichuan', 'xizang', 'shanghai',
             'liaoning', 'shanxi_S', 'jiangxi', 'fujian', 'heibei', 'yunnan']

count = 0
for i in area_list:
    count += 1
    status_train.loc[status_train["address"] == i, "address"] = count
...
print(status_train.head(10))# 查看用0，1，2替换S,C,Q之后的dataframe前5行展示
...
predictors = ["status", "address"]
# n_estimators：决策树的数量
# min_samples_split：单个分叉包含的最小行数
# min_samples_leaf：单个叶节点的最小样本数量
alg = RandomForestClassifier(random_state=1, n_estimators=40, min_samples_split=2, min_samples_leaf=1)

# 计算交叉验证的得分
kf = cross_validation.KFold(status_train.shape[0], n_folds=3, random_state=1)
scores = cross_validation.cross_val_score(alg, status_train[predictors], status_train["traffic"], cv=kf)
...
# 随机森林的参数调整
# 创建随机森林分类器对象alg
alg = RandomForestClassifier(random_state=1, n_estimators=40, min_samples_split=4, min_samples_leaf=2)
...
"""
# # 生成新特征列：家庭规模
# status_train["FamilySize"] = status_train["SibSp"] + status_train["Parch"]
# #print(titanic["FamilySize"])
# #print("\n---------------------------------以上输出新特征列:家庭规模----------------------------------------\n")
# 用.apply()方法生成新特征列：姓名长度
status_train["UrlLength"] = status_train["url"].apply(lambda x: len(x))
print(status_train["UrlLength"])
print("\n---------------------------------以上输出新特征列:url长度----------------------------------------\n")
...
predictors = ["UrlLength", "status", "address"]

# 特征选择
selector = SelectKBest(f_classif, k="all")
selector.fit(status_train[predictors], status_train["traffic"])
...
# 只选取最好的四个特征
predictors = ["UrlLength"]

alg = RandomForestClassifier(random_state=1, n_estimators=50, min_samples_split=8, min_samples_leaf=4)

# 计算交叉验证得分
scores = model_selection.cross_val_score(alg, status_train[predictors], status_train["traffic"], cv=3)
print(scores)
print("\n-----------------------------------计算交叉验证得分--------------------------------------\n")
#输出交叉验证得分的均值
print(scores.mean())
...
algorithms = [
    [GradientBoostingClassifier(random_state=1, n_estimators=25, max_depth=3),
     ["UrlLength", "status", "address"]],
    [LogisticRegression(random_state=1), ["UrlLength", "status", "address"]]
]

# # 初始化交叉验证
kf = KFold(status_train.shape[0], n_folds=3, random_state=1)

predictions = []
for train, test in kf:
    train_target = status_train["traffic"].iloc[train]
    full_test_predictions = []
#     # 对每个交叉验证分组，分别使用两种算法进行分类
    for alg, predictors in algorithms:
#         # 用训练集拟合算法
        alg.fit(status_train[predictors].iloc[train,:], train_target)
#         # 选择并预测测试集上的输出
#         # .astype(float) 可以把dataframe转换为浮点数类型
        test_predictions = alg.predict_proba(status_train[predictors].iloc[test,:])
...
#print(predictions)
# 与训练集比较以计算精度
accuracy = sum(predictions[predictions == status_train["traffic"]]) / len(predictions)
...
# 10. 测试集数据清洗
# """
titanic_test = pd.read_csv("/Users/chandler/Desktop/status_test/day=20130530/test_copy.csv")
count = 0
for i in area_list:
    count += 1
    titanic_test.loc[titanic_test["address"] == i, "address"] = count

# 添加“姓名长度”列
titanic_test["UrlLength"] = titanic_test["url"].apply(lambda x: pd.to_numeric(len(str(x)), errors='ignore'))
#
# """
# 11. 测试集机器学习
# """
...
for alg, predictors in algorithms:
    # 用训练集拟合模型
    alg.fit(status_train[predictors], status_train["traffic"])
    # 将所有数据转换为浮点数，用测试集做预测
    predictions = alg.predict_proba(titanic_test[predictors])
